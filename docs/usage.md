...
2. 「公共模板」表默认列出**所有已启用订阅源**的条目（名称、作者、来源、版本、日期、状态），按上游日期倒序，表头可点排序，可搜索名称 / 作者 / 来源，可按「全部 / 已导入 / 有更新」筛选。用「订阅源」下拉切到单个来源时，表格多出「订阅源」列，并标出该源是按 `tpls_history.json` 清单读的还是扫描文件得到的。停用的订阅不进汇总。
