...
1. 在「模板」页点标题右侧的「添加订阅」，弹窗里填订阅名称与库地址（例如 `https://github.com/qd-today/templates`）。没有导入方式的选择：订阅只是登记一个来源目录，**不会**自动把模板拉进本地——官方库里是几百个模板，全量导入会把你自己建的模板淹掉。模板是否导入、何时导入，由你在下方的「公共模板」列表里自己决定。订阅区域就在公共模板列表下方，行内可随时「编辑 / 启用停用 / 删除」。
...
5. 需要一次预载多个时，点工具栏「批量选择」才展开勾选列与批量导入工具条。**批量导入是直接的，不逐个预览**；汇总视图下勾选可以跨来源，服务端按来源分组调用。入口列会随状态在「订阅 / 更新 / 打开」之间切一个主操作。
