...
4. 模板页提供两种导入来源：从本地导入 QD HAR，或订阅一个模板库后按需挑选（见[订阅模板库](#订阅模板库选择性导入)）；本站公开模板经 PushRequest 审批后发布。
...
支持订阅官方模板库 <https://github.com/qd-today/templates>，以及任何兼容同一规范的第三方库或单个模板文件 URL。库浏览是**模板页内的一块**，不是独立页面：不再有整页跳转，也就没有「返回模板」这一步。
...
2. 同页下方的「公共模板」列出所订阅来源的条目（名称、作者、版本、日期、状态），默认按上游日期倒序，表头可点排序，可搜索名称 / 作者，可按「全部 / 已导入 / 有更新」筛选。有多个订阅源时用「订阅源」下拉切换，订阅面板里的「浏览」把某个订阅设为当前来源。
3. **点行内「订阅」即导入并打开编辑器**：导入结果会带回新建（或原地更新）模板的 id，编辑器直接打开刚拿到的模板，调好保存就出现在「我的模板」里。已导入且无更新的行显示「打开」，直接编辑本地那份。
4. 已导入的条目标「已导入」；上游版本变新时标「有更新」，点「更新」即**原地更新**，不会生成重名副本。
5. 需要一次预载多个时，点工具栏「批量选择」才展开勾选列与批量导入工具条——「全部导入」模式的来源本来就会自动全量，勾选只是例外路径。
...
对应接口：`GET /api/v1/subscriptions/{id}/library`（列目录）与 `POST /api/v1/subscriptions/{id}/import`（导入所选，逐条返回成功 / 更新 / 失败，并给出写入模板的 id 与是否原地更新）。
