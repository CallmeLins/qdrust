...
这两步都是**破坏性**的：原先依赖内网的通知渠道、订阅源与自建 DdddOCR 服务需要开启开关，且按本 ADR 的要求一律不跟随重定向（逐跳复检仍是未实现项，见上文）。OIDC 身份提供方、SMTP 投递、浏览器插件的 CDP 端点（`QDRUST_BROWSER_URL`）与 Redis / 数据库连接不在范围内——目标都由部署者写死，模板改不动（浏览器插件的 `_browser_url` 覆盖会被拒）。
